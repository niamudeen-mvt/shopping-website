...
const TestimonialSection = () => {
  ...
  return (
    <Box className="common_margin" minHeight={'747px'} bgcolor={'#f8f1ed'} padding={'74px 0'}>
        ...
        <Grid container spacing={2} mt={5}>
          {testimonialData?.length
            ? testimonialData?.map((e) => {
              return (
                <Grid item xs={12} sm={6} md={4} sx={flexCenterStyles}>
                  <Box maxWidth={'388px'} minHeight={'358px'} bgcolor={'white'} padding={'30px'} display={'grid'} gap={'10px'}>
                    <CustomRating count={e?.ratingCount} size="medium" color="#FF9000" />
                    <Typography variant="body2" lineHeight={'30px'} color="#333333">{e?.feedback}</Typography>
                    <Typography variant="body1" fontWeight={500} lineHeight={'26.5px'}>_____ {e?.author}</Typography>
                  </Box>
                </Grid>
              );
            })
